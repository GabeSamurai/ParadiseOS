...
4.  The idea of writing primitives as "create garden create tower move tower garden enter garden enter tower" in one go is rejected. You cant move something it does not exist yet, neither enter something outside your
    reach. This system must simulate the interactive physical capabilities of a common person, which includes it's limitations. Aside from the appearances, the primitives represent the actions possible by the human being      and not textual instructions. The primitives must be manual as the hands of a person.  
